...
const prepareData = (data, year, month, notSliced, usersFromClient) => {
    const workedData = [...data];
    const User = mongoose.model('User', userSchema);
    if (usersFromClient) {
        usersFromClient.forEach(async userFromClient => {
        const finded = data.some(u => u.id === userFromClient.id);
        if (!finded && !userFromClient.is_bot && userFromClient.name !== 'slackbot') {
            const newUser = new User({
                id: userFromClient.id,
                name: userFromClient.real_name,
...
export const getSendKudosData = (users, year, month, usersFromClient) => {
    const workedData = [...users];
    const fields = [{
        type: 'section',
        fields: []
    }];
    ...
    if (usersFromClient) {
        usersFromClient.forEach(async userFromClient => {
        const finded = users.some(u => u.id === userFromClient.id);
        if (!finded && !userFromClient.is_bot && userFromClient.name !== 'slackbot') {
            const newUser = new User({
                id: userFromClient.id,
                name: userFromClient.real_name,
    ...
    workedData.sort((a, b) => {
        const check1 = a && a?.reactions_added && a.reactions_added[year] && a.reactions_added[year][month] || 0;
        const check2 = b && b?.reactions_added && b.reactions_added[year] && b.reactions_added[year][month] || 0;
        if (!check1 && !check2) {
            return 0;
        }
        if (!check1) {
